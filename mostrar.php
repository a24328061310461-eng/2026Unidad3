...
<div class="table-container">
    ...
    $sql = "SELECT id, nombrereal, personaje, altura, peso, poderes, sexo, debilidad, creation, biografia FROM personajes";
    $resultado = $conexion->query($sql);

    if ($resultado->num_rows > 0) {
        echo '<div style="text-align: center;"><span class="badge-count">Total: ' . $resultado->num_rows . ' personajes</span></div>';
        ...
        while ($row = $resultado->fetch_assoc()) {
            echo "<tr>
                    ...
                    <td>" . nl2br(htmlspecialchars(substr($row["biografia"] ?? '', 0, 100))) . (strlen($row["biografia"] ?? '') > 100 ? '...' : '') . "</td>
                  </tr>";
        }
